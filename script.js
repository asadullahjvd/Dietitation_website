document.addEventListener('DOMContentLoaded', function () {
    ...
    /* =========================================================
       Header height (used to offset scroll position, since the
       header is fixed on every page)
    ========================================================= */
    ...
    /* =========================================================
       Smooth scrolling for in-page anchor links (e.g. "#calculator")
       Links that point to another page (e.g. services.html) are
       left alone so the browser navigates normally.
    ========================================================= */
    document.querySelectorAll('a[href*="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            // Only intercept links that point to an anchor on THIS page
            const isSamePage = path === '' || path === window.location.pathname.split('/').pop();
            if (!isSamePage || hash.length <= 1) return;
    ...
    // (e.g. arriving at index.html#calculator from another page),
    // scroll to the right spot once the layout has settled.
    ...
    /* =========================================================
       BMI / BMR / Calorie Calculator (index.html only)
    ========================================================= */
    const calculateBtn = document.getElementById('calculate-btn');
    const resetBtn = document.getElementById('reset-btn');
    const resultsSection = document.getElementById('results-section');

    if (calculateBtn && resultsSection) {
        ...
        formulaRadios.forEach(radio => {
            radio.addEventListener('change', function () {
                ...
                const imperialWeight = document.querySelector('.imperial-weight');
    ...
    function calculateBMI() {
        ...
        // Calculate BMR (Basal Metabolic Rate) using Harris-Benedict
        let bmr;
        if (gender === 'male') {
    ...
    /* =========================================================
       Appointment form -> WhatsApp (contact.html only)
    ========================================================= */
    ...
    if (whatsappBtn) {
        whatsappBtn.addEventListener('click', function () {
            ...
            const phone = encodeURIComponent(document.getElementById('phone').value || 'Not provided');
            const service = document.getElementById('service').value;
            const message = encodeURIComponent(document.getElementById('message').value);

            // Validate required fields
            if (!document.getElementById('name').value || !document.getElementById('email').value ||
                !service || !document.getElementById('message').value) {
            ...
            const whatsappMessage =
                ...
                `*Email:* ${email}` + '%0A' +
                `*Phone:* ${phone}` + '%0A' +
                `*Service:* ${encodedService}` + '%0A%0A' +
                `*Message:*` + '%0A' +
                `${message}`;

            window.open(`https://wa.me/923017996736?text=${whatsappMessage}`, '_blank');
        });
    }

    // Fallback native submit handler (in case JS validation is bypassed)
    ...
    /* =========================================================
       Responsive navigation for mobile (all pages)
    ========================================================= */
    const headerContainer = document.querySelector('header .container');
    const nav = document.querySelector('nav');

    if (headerContainer && nav && !headerContainer.querySelector('.mobile-nav-toggle')) {
